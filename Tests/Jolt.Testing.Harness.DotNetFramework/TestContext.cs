...
namespace Jolt.Testing.Harness.DotNetFramework
{
    public class TestContext : ITestContext
    {
        ...
        public IJsonObject ParseAsJsonObject(TestType testType, string json)
        {
            switch (testType)
            {
                case TestType.Newtonsoft: return CreateNewtonsoftContext().JsonTokenReader.Read(json) as IJsonObject;
                case TestType.DotNet: return CreateDotNetContext().JsonTokenReader.Read(json) as IJsonObject;
                default: throw new ArgumentOutOfRangeException(nameof(testType), testType, null);
            };
        }
    }
