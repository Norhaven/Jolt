...
We have several flavors of test in use here: `Transformer`, `JSON`, and `Small`. These all live in the `Jolt.Testing` project, so let's go over those here.

## Transformer

These tests exist in the `./Transformers` area of the project and are the most typical way of testing that you are probably already familiar with. Each is tagged with a `TransformerTest` attribute which specifies the transformer file name and source document file name that this test will make use of. It will then make some assertions about the state of the objects it receives as a result of transforming the source JSON. The files are located in the `Jolt.Testing` project, in the `./Resources/TestFiles/Transformers` and `./Resources/TestFiles/Documents` folders respectively. If you have a lot of expectations that should be verified about a given test result, this may be a good place for that.

## JSON

These tests exist in the `./Json` area of the project and are tagged with a `JsonTest` attribute which specifies the tests file used for the given set of tests and expectations. The entire test structure exists in the `Jolt.Testing` project, in the `./Resources/TestFiles/JsonTests` folder. The layout of the JSON tests in their files should hopefully be straightforward to understand, with a little bit of documentation about the [JSON Test schema](https://github.com/Norhaven/Jolt/blob/main/Tests/Jolt.Testing/Resources/TestFiles/JsonTests/SchemaDocs/README.md) as well (generated by `jsonschema2md`). Generally, these are groups of like-minded tests that operate on medium-sized transformers and operations.

## Small

These tests exist in the `./Small` area of the project and are tagged with a `SmallTest` attribute and additionally decorated with more attributes describing the source input, the transformer snippet, and the expectations for the shape of its results. As a rule of thumb, if a test involves variations on a single method call or operation then it belongs here.
